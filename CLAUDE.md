...
- **Interceptors fly via gz `VelocityControl`, not a multicopter thrust controller.** The original `MulticopterVelocityControl` was non-deterministic (only ~1 of 3 controllers ever latched `enable`) and unstable (the one that activated stalled and flew off-field to ~800 m). It was replaced by `gz::sim::systems::VelocityControl` per model: physics applies the driver's `cmd_vel` directly to the rigid body — reliable, continuous, with collisions/inertia but no rotor-thrust dynamics. The `MulticopterMotorModel`/`MulticopterVelocityControl` plugins and their `maxLinearAcceleration`/`maximumLinearVelocity` params are gone from `intercept_scenario.sdf` (the rotor links remain as visual-only geometry).
- **`cmd_vel` linear velocity is body-frame** — the driver rotates the desired world velocity by −yaw before publishing. Scenario speeds (shaheds 5–8 m/s, interceptors 13 m/s) are tuned for a readable demo, not a flight envelope.
- **`VelocityControl` has no enable handshake** — it keeps applying the last `cmd_vel` it received every step, overriding gravity. The driver commands a **zero twist for expended interceptors** so they freeze in place instead of coasting forever on the last command.
