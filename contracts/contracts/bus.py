"""
Shared transport — the `Bus` protocol and its concrete implementations.

This lives in ``contracts`` (the shared foundation) so every package depends only
on ``contracts`` for messaging — no inter-team package dependency. ``agent.bus``
re-exports these names for backwards compatibility.

The consensus / re-tasking logic depends only on the `Bus` protocol
(``publish`` + ``subscribe``) — never on a concrete transport. Implementations
provided: ``MockBus`` (in-process), ``ROS2Bus`` (DDS), ``ZmqBus`` (ZeroMQ), and
``SplitBus`` (subscribe on one transport, publish on another).
...
class ZmqBus:
    ...
    def spin(self, timeout_ms: int = 100, *, max_messages: int = 256) -> int:
        """Deliver waiting messages to handlers; returns the number handled.

        Drains at most ``max_messages`` frames per call, so a high-volume sender
        cannot starve the caller's loop — it yields and ticks again next pass."""
        if self._sub is None:
            return 0
        handled = 0
        while handled < max_messages and self._sub.poll(timeout=timeout_ms):
...
# ---------------------------------------------------------------------------
# SplitBus — receive on one transport, send on another
# ---------------------------------------------------------------------------


class SplitBus:
    """A ``Bus`` that subscribes on one transport and publishes on another.

    A node that receives on one ZeroMQ endpoint and emits on a different one
    (a single ``ZmqBus`` cannot bind its SUB and PUB to the same address) wires
    an inbound bus and an outbound bus together through this adapter.
    """
