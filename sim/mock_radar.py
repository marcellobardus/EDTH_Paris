...
def _detect(
    radar_cfg, shahed: _Shahed, rng: random.Random, radar_id: str, t: float, drop_prob: float
) -> RadarDetection | None:
    if not shahed.alive:
        return None
...
def _false_positive(radar_cfg, rng: random.Random, radar_id: str, t: float) -> RadarDetection:
...
def run_stdout(
    cfg: ScenarioConfig, rate_hz: float, seed: int, fp_prob: float, drop_prob: float
) -> None:
...
def run_ros2(
    ...
    class MockRadarNode(Node):
        def __init__(self):
            ...
            self.get_logger().info(f"publishing on {Topics.RADAR_DETECTIONS} at {rate_hz} Hz")

        def _tick(self):
            dets = _tick(
                cfg, self._shaheds, self._rng, "radar_", self._t, self._dt, fp_prob, drop_prob
            )
            for det in dets:
...
def main() -> None:
    ...
    parser.add_argument(
        "--false-positive-prob", type=float, default=DEFAULT_FP_PROB, dest="fp_prob", metavar="PROB"
    )
    parser.add_argument(
        "--drop-prob", type=float, default=DEFAULT_DROP_PROB, dest="drop_prob", metavar="PROB"
    )
