...
class LiveTracker:
    ...
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.labels: OrderedDict[str, tuple[str, tuple]] = (
            OrderedDict()
        )  # track_id -> (name, colour)
    ...
    def render(self, ax) -> None:
        ...
        active = (
            ", ".join(
                name
                for name, _ in self.labels.values()
                if name in {n for n, _ in (self.labels[t.track_id] for t in self.live)}
            )
            or "—"
        )
        ax.set_title(
...
def main() -> None:
    ...
    p.add_argument(
        "--cull", type=float, default=5500.0, help="range (m) past which drones are dropped"
    )
    ...
    if args.headless:
        ...
        print(
            f"[headless] ran {args.frames} ticks, "
            f"classified {len(app.labels)} threats -> {args.out}"
        )
        return
